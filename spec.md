...
- **Lượt 2 (chiều 17/9, trước CP4) — chạy lại tự động qua script gọi thẳng `/generate`, MỚI CHỈ đo ở mức HTTP (qua/không qua validate), CHƯA chấm nội dung từng case theo đúng kỳ vọng như lượt 1** (việc chấm nội dung 15 case pass để lại làm tiếp trước CP6):
  - **15/20 qua validate (HTTP 200)** · **1/20 đúng kỳ vọng robustness (case 20, HTTP 400)** · **4/20 fail (HTTP 502): case 4, 6, 7, 16**.
  - Ngay lượt chạy thử case 1 đầu tiên đã lộ 1 **bug thật mới** do chính code hôm nay gây ra: Layer 4b (mở rộng kiểm số ở `chuTrenManHinh`) chấm oan số do CHÍNH NGƯỜI DÙNG gõ trong mục tiêu (VD "năm 2024") là số liệu bịa, vì câu dẫn nhập không có trích dẫn nên không có gì để đối chiếu. Đã sửa: cho phép số trùng với chính topic/goal/audience/duration người dùng nhập, không tính là bịa. Case 1 sau khi sửa: HTTP 200, đúng hành vi mong đợi (không bịa %).
  - Case 6 fail đúng ý đồ: AI cố đưa giá token cụ thể ("5.2", "5.6") không có nguồn — Layer 4b chặn đúng.
  - Case 4, 7, 16 fail vì **Layer 7** (AI judge chấm trích dẫn không thực sự liên quan) — đây là hành vi CHƯA từng thấy ở lượt 1 (lúc đó chưa có Layer 7). Chưa rõ đây là Layer 7 đúng đắn phát hiện trích dẫn yếu thật, hay đang chấm gắt hơn cần thiết — cần đọc kỹ nội dung từng case mới kết luận được, để lại làm tiếp trước CP6.

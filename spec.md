...
- Validation với user:
  - **Validation chính cần tiếp tục:** Lab Coach/Studio team/giảng viên vẫn là user chính của C3, vì họ mới đánh giá được workflow viết kịch bản, duyệt nguồn, loại nguồn và viết lại phần liên quan. Nhóm đã có Thành Phạm (Lab Coach kiêm Studio team, cũng là 1 trong 2 người phỏng vấn ở `eval/interview-guide.md`) đồng ý thử prototype, nhưng chưa đủ một vòng log độc lập với ≥2 Studio/Lab Coach cho workflow chính.
  - **Validation phụ đã làm với học viên:** 2 học viên ngoài nhóm (**Trướng**, **Khải**) đọc/xem đoạn kịch bản ngắn để kiểm tra độ dễ hiểu của đầu ra, log ở `validation/learner-check.md`. Kết quả: 2/2 hiểu thông điệp chính về kiểm chứng nguồn; cả 2 đều đề xuất cần biểu diễn trực quan hơn trạng thái xác minh/câu cần nguồn. Quyết định sản phẩm: giữ nhãn `da-xac-minh`/`chua-xac-minh`/`nguon: []`, và ưu tiên UI bấm câu → thấy trạng thái xác minh → mở đoạn trích nguồn.
  - Giới hạn: learner validation này chỉ kiểm chất lượng lời giảng/dễ hiểu với người học, **không thay thế** validation chính của workflow Studio team.
...
| CP4 (17/9, tối) | UI: tự động gọi `/expand-script` ngay sau khi `/generate` xong, không bắt bấm tay nút "Làm phong phú kịch bản" nữa | Nút nằm cuối trang, người dùng thật không biết để bấm nên tưởng hệ thống luôn trả kịch bản ngắn — vẫn giữ nguyên guard tự rớt về bản gốc nếu AI vi phạm luật |
...
| CP4 (17/9, tối) | **Thêm Layer 8** — guardrail độc lập `check_topic_in_scope()`, chạy TRƯỚC web-search + generate chính, chặn cứng chủ đề ngoài phạm vi AI/công nghệ bằng `HTTPException(400)`. Đồng thời bỏ đoạn hướng dẫn "kiểm tra phạm vi" đã vô hiệu ra khỏi `PROMPT_TEMPLATE` chính | Chạy lại golden-set (Case 27) phát hiện case 5 tái hiện NẶNG HƠN: AI tìm được nguồn THẬT về nấu phở (vì web-search luôn tìm đúng từ khoá) rồi viết hẳn kịch bản nấu ăn hoàn chỉnh — không lớp nào cũ bắt được vì không có gì bịa. Sửa bằng guardrail AI-judge riêng (kiểu Layer 7) thay vì chỉ sửa prompt chính, theo đúng góp ý "nên dùng guardrail". Test lại: case 5 chặn đúng trong <3s, case 1 (đúng phạm vi) không bị ảnh hưởng |
| CP4 (17/9, tối, sau khi nộp) | Sửa Layer 7: chỉ raise lỗi khi `soNguonXacNhan >= 2` (đúng ý đồ gốc: bắt khai khống SỐ NGUỒN, không phải chấm độ chuẩn của 1 trích dẫn đơn) | Case 27 phát hiện Layer 7 chấm oan hàng loạt case chỉ có 1 trích dẫn (case 1, 4, 7, 16 fail 502 dù không hề bịa). Test lại: cả 4 case chuyển pass, case 6 (đúng phải fail vì bịa giá) không bị ảnh hưởng |
| CP4 (17/9, tối, sau khi nộp) | Thêm "ĐẦY ĐỦ Ý" vào `PROMPT_TEMPLATE`: nếu 1 trang liệt kê nhiều mục cùng loại, phải trích đủ từng mục, không gộp/lược bớt | Case 8 thiếu hẳn "Deep Learning" trong chuỗi AI⊃ML⊃DL⊃GenAI⊃LLM. Test lại: đủ cả 5 khái niệm đúng thứ tự, case 1/17 không bị ảnh hưởng. Đánh đổi: case 9 (slide ít mốc) thử thêm 1 mốc bịa sau hướng dẫn này, bị Layer 4b chặn đúng — xem case 29 `eval/golden-set.md` |
| CP4 (17/9, tối, sau khi nộp) | Thêm "ĐÚNG ĐỘ SÂU THEO ĐỐI TƯỢNG" vào `PROMPT_TEMPLATE`: đối tượng chuyên môn cao phải đào sâu hơn; thuật ngữ tiếng Anh phải giải nghĩa tiếng Việt ở lần nhắc đầu | Case 18 (chưa giải nghĩa PAIR) → sửa xong. Case 19 (chưa đào sâu Transformer cho giảng viên) → cải thiện một phần (nguồn khai thác sâu hơn) nhưng lời đọc vẫn chưa thuật lại đủ chi tiết kỹ thuật, chưa giải quyết triệt để |
| — (bonus, sau CP4) | Thử nghiệm `render_video_hyperframes.py`: dựng video bằng HyperFrames — AI tự thiết kế sơ đồ minh hoạ đúng cơ chế đang nói (không phải hình trang trí), phong cách 3Blue1Brown (nền gần đen, hoạt ảnh vẽ dần bằng SVG stroke), retry 3 lần + validate bằng `hyperframes check` trước khi rớt về template an toàn | Video ảnh tĩnh cũ (`render_video.py`) không đổi, đây là nhánh thử nghiệm riêng. Test thật: nhiều cảnh (cắt token, attention, giới hạn context) đều ra đúng sơ đồ khớp nội dung, xem `demo-video-hyperframes-v2.mp4`/`demo-video-hyperframes-3b1b-test.mp4` |
| — (bonus, sau CP4) | Thêm `expand_script_to_target()` + endpoint `/expand-script-full`: gọi lặp nhiều vòng `expand_script_with_analogy()` để cố đạt gần số câu mục tiêu theo thời lượng, kèm phong cách giảng giải 3Blue1Brown (xây trực giác "tại sao", 1 ví dụ liên tưởng xuyên suốt, dẫn dắt bằng câu hỏi) | Test thật: KHÔNG đạt target (5→14-20/34 câu, 41-59%) vì gpt-4o-mini giảm độ tin cậy JSON sau ~3 vòng gọi liên tiếp (nhét lời đọc vào field "kieu", để trống "loi") — đã thêm Guard 2b chặn đúng lỗi này thay vì nới lỏng an toàn để cố đạt đủ số câu. Xem case 31 `eval/golden-set.md` |
| — (bonus, sau CP4) | Đổi `expand_script_with_analogy()` sang OpenAI Structured Outputs (`json_schema` + `strict:true`) thay vì `json_object` — ép field `"kieu"` theo đúng enum, không còn chỗ để model nhét nhầm câu chữ vào | Sửa đúng triệt để lỗi field ở case 31 (0/6 lần lặp lại lỗi cũ), NHƯNG lộ ra lỗi khác vẫn tồn tại: ~1/3 lần Guard 1 (giữ nguyên câu gốc) chặn vì model tái tạo không khớp tuyệt đối từng ký tự — tỉ lệ đạt gần target 34 câu không cải thiện (2/3 lần chạy full thất bại ngay vòng 1, 1/3 lần vẫn đạt ~20 câu như trước). Xem case 32 `eval/golden-set.md` |
| — (bonus, sau CP4) | Đổi model bước expand từ `gpt-4o-mini` sang `gpt-5.4-mini` (chỉ bước này, `/generate` chính không đổi) | Test thật: 5/5 lần pass Guard 1 (trước ~2/3), full `/expand-script-full` 3/3 lần đều thành công có tiến triển (trước 2/3 lần thất bại ngay vòng 1) — tốt nhất đạt 30/34 câu = 88% (trước 41-59%). Đắt hơn ~5-7,5x nhưng chi phí tuyệt đối vẫn nhỏ (vài cent/video). Xem case 33 `eval/golden-set.md` |
| — (bonus, 18/9) | `render_video_hyperframes.py`: tách "khung thương hiệu" (eyebrow/accent-bar/tiêu đề/khung phụ đề/nhãn nguồn) thành phần DO CODE RENDER CỐ ĐỊNH dùng chung mọi cảnh, thêm quy tắc bắt buộc "trọng lượng thị giác" (thẻ/card có nền/viền, cấm chữ trần), đổi model thiết kế cảnh sang `gpt-5.4-mini` | So sánh trực tiếp video mẫu bên ngoài (`transformer_explainer.mp4`) phát hiện bố cục cũ trống trải/rời rạc. Test trước/sau cùng 1 câu: bản cũ chỉ ra 1 khung chữ nhật trống, bản mới ra hẳn hình minh hoạ + thẻ nhãn + sơ đồ phụ. Xem case 34-35 `eval/golden-set.md` |
| — (bonus, 18/9) | Vòng tự-sửa 2 tầng: `lint --json` (nhanh, không mở trình duyệt) rồi `check --json` (đầy đủ, bắt lỗi layout/contrast/runtime) — feed đúng lỗi thật cho AI vá thay vì sinh lại mù. Thêm tiếp tầng 3: **AI chấm chất lượng sáng tạo bằng ảnh chụp thật** (model khác — `gpt-4o` — theo nguyên tắc Dual-Model Separation), bắt lỗi mà check kỹ thuật không thấy (ô trống không chữ, bố cục rối, sơ đồ lạc đề) | Phát hiện thật: 1 cảnh pass `check` sạch ngay lần đầu nhưng có 2 ô trống không chữ — check kỹ thuật không có cách nào bắt loại lỗi này. Thêm tầng 3 xác nhận bắt đúng và tự sinh lại tới khi đạt |
| — (bonus, 18/9) | Đổi bảng màu/phong cách HyperFrames từ "3Blue1Brown dark" sang **"VinUni Academic Light Theme"** + cấm tuyệt đối emoji hệ điều hành — theo skill `scriptscout-authoring` của teammate Nguyễn Việt Dũng (`github.com/DungBallad/ScoutX-Skills`, cài vào `.claude/skills/` của repo) | Đội trưởng xác nhận đổi theo yêu cầu. Test thật: 48/48 rồi 47/47 lượt kiểm tương phản đều đạt WCAG AA với bảng màu mới |
| — (bonus, 18/9) | **Tích hợp agent code thật** (`claude -p` headless, hàm `generate_scene_via_claude_code()`) làm tầng 0 — thử TRƯỚC toàn bộ pipeline API, rớt về đường cũ nếu lỗi/timeout/không có CLI. Agent tự đọc skill, tự viết file, tự chạy `hyperframes check`, tự sửa lặp lại — khác hẳn 1 lượt gọi API hoàn thiện | Test trực tiếp: pass sạch ngay lần thử đầu, 47/47 WCAG AA, chất lượng vượt hẳn đường API thuần (so sánh ảnh chụp trực tiếp). **Giới hạn thật đã ghi rõ trong code**: dùng tài khoản Pro cá nhân qua CLI, KHÔNG phù hợp sản phẩm nhiều người dùng đồng thời (cần Anthropic API key riêng để lên production thật), mỗi lượt cũng chậm hơn nhiều (~1-4 phút/cảnh) — giảm `max_workers` mặc định 6→3 vì lý do này |
